/*
* @Author: Xiao
* @Date:   2019-01-12 10:29:05
* @Last Modified by:   Xiao
* @Last Modified time: 2019-01-16 10:38:08
*/
'use strict';
require('./index.css');
...
var page = {
		...
		$(document).on('click', '.cart-delete', function() {
			if(window.confirm('确认删除该商品？')) {
				var productId = $(this).parents('.cart-table')
					.data('product-id');
				_this.deleCartProduct(productId);
			}
		})
		// 删除选中商品
		$(document).on('click', '.delete-selected', function() {
			if(window.confirm('确认删除选中的商品？')) {
				...
				if (arrProductIds.length) {
					_this.deleCartProduct(arrProductIds.join(','));
				}
				else {
	...
	deleCartProduct: function(productIds) {
		var _this = this;
	...
	showCartError: function() {
		$('.page-wrap').html('<p class="err-tip">哪里不对了，刷新下试试哟~</p>')
	}
